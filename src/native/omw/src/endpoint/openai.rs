//! The OpenAI-compatible HTTP endpoint, behind the `endpoint-openai`
//! feature. When `[endpoint]` is configured with `kind = "openai"`, a small
//! axum server is started exposing `/v1/models` and `POST
