...
impl Provider for EchoProvider {
  ...
  async fn chat_stream(
    ...
    Ok(Box::pin(futures_util::stream::iter(responses.into_iter().map(
      |content| {
        Ok(ChatDelta {
...
"#;
  ...
  let entry = entries.get("echo").ok_or_else(|| {
    anyhow::anyhow!("missing provider \"echo\"")
  })?;
  anyhow::ensure!(
  ...
  let result =
    provider.chat("echo-model", messages.clone(), Vec::new()).await?;
  anyhow::ensure!(
  ...
  let mut stream =
    provider.chat_stream("echo-model", messages.clone(), Vec::new()).await?;
